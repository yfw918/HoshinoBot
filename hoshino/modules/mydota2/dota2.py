...
class Player:
    def __init__(self, account_id, last_match, personaname='') -> None:
...
async def broadcast(player: Player, bot):
    try:
        ...
        print(f'获取{player.account_id}的last_match失败: {e}')
    ...
    if last_match != player.last_match:
        ...
        try:
            print(f"获取{account_id}的personaname")
            personaname = player.personaname if player.personaname else get_url(f"{url_prefix}/ISteamUser/GetPlayerSummaries/v2?key={apikey}&steamids={account_id+76561197960265728}").json()['response']['players'][0]['personaname']
        except Exception as e:
            print(f'获取{account_id}的personaname失败: {e}')
...
def run_in_thread(player, bot):
    asyncio.run(broadcast(player, bot))
